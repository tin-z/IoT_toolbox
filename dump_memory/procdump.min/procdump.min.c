...
/* author: tin-z
 * took inspiration from https://unix.stackexchange.com/questions/6301/how-do-i-read-from-proc-pid-mem-under-linux
 *
 * Usefull for old kernel version not supporting 'process_vm_readv' syscall and similars
 * */
  ...
  if (mem_fd > 0) {
    ...
    if ( rets = lseek(mem_fd, offset, SEEK_SET), rets >= 0) {
      ...
      if ( rets = read(mem_fd, buf, length), rets >= 0) {
        ...
        if (fwrite(buf, 1, length, write_fd) >= 0) {
